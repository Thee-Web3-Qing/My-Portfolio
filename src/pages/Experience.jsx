// From Giwa's CV — actual roles and events
const roles = [
  {
    period: '2025 — Present',
  ...
  return (
    <div className="pt-12 pb-24">
      ...
      <section className="mb-20">
        <h2 className="font-display text-2xl font-bold mb-6">Awards & recognition</h2>
        <div className="grid sm:grid-cols-2 gap-4">
          {awards.map((a, i) => (
            <div key={i} className="noise-card border border-ink rounded-blob p-5 bg-white/60">
              <span className="font-mono text-xs uppercase text-coral">Award</span>
              <p className="font-display text-lg font-bold mt-2">{a}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ROLES */}
      <section className="mb-20">
        <h2 className="font-display text-2xl font-bold mb-6">Roles & positions</h2>
        <div className="space-y-6">
          {roles.map((r, i) => (
            <div key={i} className="grid sm:grid-cols-[140px_1fr] gap-4 border-t border-line pt-6">
              <span className="font-mono text-xs uppercase text-mid">{r.period}</span>
              <div>
                <h3 className="font-display text-lg font-bold">
                  {r.title} <span className="text-mid font-body font-normal">— {r.org}</span>
                </h3>
                <p className="text-sm text-mid mt-1">{r.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* EVENTS */}
      <section>
        <h2 className="font-display text-2xl font-bold mb-6">Events hosted & convened</h2>
        <div className="space-y-6">
          {events.map((e, i) => (
            <a
              key={i}
            ...
            >
              <span className="font-mono text-xs uppercase text-coral">{e.role} · {e.date}</span>
              <div>
                <h3 className="font-display text-lg font-bold group-hover:underline">{e.name}</h3>
                <p className="text-sm text-mid mt-1">{e.desc}</p>
              </div>
            </a>
          ))}
        </div>
